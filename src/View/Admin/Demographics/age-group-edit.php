...
<div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
    <form action="/admin/demographic/age-group/edit" method="POST">
        <div class="p-6">
            <div class="flex items-center justify-between mb-4">
                <h3 class="font-semibold text-slate-700">Data Kelompok Umur</h3>
                <button type="button" onclick="addAgeGroupRow()" 
            ...
            <div id="ageGroupContainer" class="space-y-3">
                <?php if (empty($model['ageGroups'])): ?>
                    <!-- Default row jika kosong -->
                    <div class="age-group-row grid grid-cols-1 md:grid-cols-2 gap-3 items-end bg-slate-50 rounded-xl p-4 border border-slate-200">
                        <div>
                            <label class="block text-xs font-medium text-slate-500 mb-1">Rentang Umur</label>
                            <input type="text" name="age_range[]" 
                                   ...
                                   placeholder="Contoh: 0-5, 6-12, 13-17..." required>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-500 mb-1">Jumlah</label>
                            <input type="number" name="age_total[]" 
                                   class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10"
                                   placeholder="0" min="0" required>
                        </div>
                        <button type="button" onclick="removeAgeGroupRow(this)" 
                                class="text-red-500 hover:text-red-700 transition p-1">
                            <iconify-icon icon="solar:trash-bin-trash-linear" width="18"></iconify-icon>
                        </button>
                    </div>
                <?php else: ?>
                    <?php foreach ($model['ageGroups'] as $ageGroup): ?>
                        <div class="age-group-row grid grid-cols-1 md:grid-cols-2 gap-3 items-end bg-slate-50 rounded-xl p-4 border border-slate-200">
                            <div>
                                <label class="block text-xs font-medium text-slate-500 mb-1">Rentang Umur</label>
                                <input type="text" name="age_range[]" 
                                       value="<?= htmlspecialchars($ageGroup->ageRange) ?>"
                                       class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10"
                                       placeholder="Contoh: 0-5, 6-12, 13-17..." required>
                            </div>
                            <div>
            ...
            <div class="mt-6 bg-slate-50 rounded-xl p-4 flex items-center justify-between">
                <span class="text-sm font-medium text-slate-600">Total Kelompok Umur</span>
                <span class="text-xl font-bold text-slate-900" id="totalAgeGroup">0</span>
            </div>
        </div>
...
<script>
    ...
    function addAgeGroupRow() {
        ...
        row.className = 'age-group-row grid grid-cols-1 md:grid-cols-2 gap-3 items-end bg-slate-50 rounded-xl p-4 border border-slate-200';
        row.innerHTML = `
            <div>
                <label class="block text-xs font-medium text-slate-500 mb-1">Rentang Umur</label>
                <input type="text" name="age_range[]" 
    ...
    // ==========================================================
    // REMOVE AGE GROUP ROW
    // ==========================================================
    function removeAgeGroupRow(btn) {
        const container = document.getElementById('ageGroupContainer');
        if (container.children.length > 1) {
            btn.closest('.age-group-row').remove();
            updateTotal();
        } else {
            showToast('⚠️ Minimal harus ada 1 data kelompok umur!', 'warning');
        }
    }
    ...
    function updateTotal() {
        const totals = document.querySelectorAll('input[name="age_total[]"]');
        let sum = 0;
        totals.forEach(input => {
            const val = parseInt(input.value) || 0;
            sum += val;
        });
        document.getElementById('totalAgeGroup').textContent = sum.toLocaleString('id-ID');
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('input[name="age_total[]"]').forEach(input => {
    ...
    document.addEventListener('input', function(e) {
        if (e.target && e.target.name === 'age_total[]') {
            updateTotal();
        }
    });
</script>