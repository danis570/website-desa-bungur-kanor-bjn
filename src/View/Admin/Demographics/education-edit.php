...
<div class="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8">
    <div>
        ...
        <p class="mt-2 text-slate-500 max-w-2xl text-sm lg:text-base leading-relaxed">
            Perbarui data demografi berdasarkan tingkat pendidikan.
        </p>
    </div>
</div>
...
<div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
    <form action="/admin/demographic/education/edit" method="POST">
        <div class="p-6">
            <div class="flex items-center justify-between mb-4">
                <h3 class="font-semibold text-slate-700">Data Pendidikan</h3>
            ...
            <div id="educationContainer" class="space-y-3">
                <?php if (empty($model['educations'])): ?>
                    ...
                    <div class="education-row grid grid-cols-1 md:grid-cols-2 gap-3 items-end bg-slate-50 rounded-xl p-4 border border-slate-200">
                        <div>
                            <label class="block text-xs font-medium text-slate-500 mb-1">Tingkat Pendidikan</label>
                        ...
                        <div>
                            ...
                            <input type="number" name="education_total[]" 
                                   class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10"
                                   placeholder="0" min="0" required>
                        </div>
                        <button type="button" onclick="removeEducationRow(this)" 
                    ...
                    <?php foreach ($model['educations'] as $education): ?>
                        <div class="education-row grid grid-cols-1 md:grid-cols-2 gap-3 items-end bg-slate-50 rounded-xl p-4 border border-slate-200">
                            <div>
                                <label class="block text-xs font-medium text-slate-500 mb-1">Tingkat Pendidikan</label>
                                <input type="text" name="education_level[]" 
                                       value="<?= htmlspecialchars($education->educationLevel) ?>"
                                       class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10"
                                       placeholder="Contoh: SD, SMP, SMA..." required>
                            </div>
                            <div>
            ...
            <div class="mt-6 bg-slate-50 rounded-xl p-4 flex items-center justify-between">
                <span class="text-sm font-medium text-slate-600">Total Pendidikan</span>
                <span class="text-xl font-bold text-slate-900" id="totalEducation">0</span>
            </div>
        </div>
...
<script>
    ...
    function addEducationRow() {
        ...
        row.className = 'education-row grid grid-cols-1 md:grid-cols-2 gap-3 items-end bg-slate-50 rounded-xl p-4 border border-slate-200';
        row.innerHTML = `
            <div>
                <label class="block text-xs font-medium text-slate-500 mb-1">Tingkat Pendidikan</label>
                <input type="text" name="education_level[]" 
    ...
    function removeEducationRow(btn) {
        ...
        if (container.children.length > 1) {
            btn.closest('.education-row').remove();
            updateTotal();
        } else {
            showToast('⚠️ Minimal harus ada 1 data pendidikan!', 'warning');
        }
    }
    ...
    function updateTotal() {
        const totals = document.querySelectorAll('input[name="education_total[]"]');
        let sum = 0;
        totals.forEach(input => {
            const val = parseInt(input.value) || 0;
            sum += val;
        });
        document.getElementById('totalEducation').textContent = sum.toLocaleString('id-ID');
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('input[name="education_total[]"]').forEach(input => {
    ...
    document.addEventListener('input', function(e) {
        if (e.target && e.target.name === 'education_total[]') {
            updateTotal();
        }
    });
</script>