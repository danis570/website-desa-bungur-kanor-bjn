...
<script>
    ...
    const allData = [
        {
            id: 1,
            year: "1923",
            title: "Awal Berdirinya Desa",
            description: "Desa Bungur mulai terbentuk sebagai kawasan permukiman masyarakat agraris yang mengandalkan pertanian sebagai sumber kehidupan utama.",
            image: "https://desasulangai.badungkab.go.id/storage/desasulangai/image/WhatsApp%20Image%202025-10-22%20at%2019.12.28_899f1a44.jpg",
            icon: "flag"
        },
        {
            id: 2,
            year: "1985",
            title: "Pembangunan Kantor Desa",
            description: "Pemerintah desa mulai membangun kantor desa sebagai pusat pelayanan masyarakat dan administrasi pemerintahan.",
            image: "https://desasulangai.badungkab.go.id/storage/desasulangai/image/WhatsApp%20Image%202025-10-22%20at%2019.12.28_899f1a44.jpg",
            icon: "building-2"
        },
        {
            ...
            description: "Penerapan teknologi pertanian mulai meningkatkan hasil panen dan kesejahteraan masyarakat Desa Bungur.",
            image: "https://desasulangai.badungkab.go.id/storage/desasulangai/image/WhatsApp%20Image%202025-10-22%20at%2019.12.28_899f1a44.jpg",
            icon: "tractor"
        },
        {
            ...
            description: "Desa Bungur terus bertransformasi melalui digitalisasi pelayanan publik, transparansi informasi, dan pengembangan website resmi desa.",
            image: "https://desasulangai.badungkab.go.id/storage/desasulangai/image/WhatsApp%20Image%202025-10-22%20at%2019.12.28_899f1a44.jpg",
            icon: "laptop"
        }
    ];
    ...
    function IconRenderer(params) {
        ...
        return `<span class="text-2xl">${icons[params.value] || icons.default}</span>`;
    }
    ...
    const columnDefs = [
        ...
        { headerName: "Icon", field: "icon", cellRenderer: IconRenderer, width: 70, minWidth: 70, maxWidth: 70, resizable: false },
    ...
    function applyFilters() {
        ...
        filteredData = allData.filter(item => {
            const matchSearch = item.year.toLowerCase().includes(keyword) ||
    ...
    function openFormModal(mode, data = null) {
        ...
        if (mode === 'add') {
            currentEditId = null;
            title.textContent = 'Tambah Timeline Sejarah';
        } else {
            currentEditId = data.id;
            title.textContent = 'Edit Timeline Sejarah';
        }
    ...
    function renderForm(data) {
        ...
        const icons = [
            ...
            { value: 'building-2', label: '🏛️ Building' },
            ...
            <form id="sejarahForm" class="p-6">
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="formDescription" rows="4"
                            class="w-full rounded-xl border border-slate-200 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-slate-900/10"
                            placeholder="Deskripsikan peristiwa sejarah..." required>${data ? data.description : ''}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="formIcon" 
                            class="w-full rounded-xl border border-slate-200 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-slate-900/10">
                            ${icons.map(icon => 
                                `<option value="${icon.value}" ${data && data.icon === icon.value ? 'selected' : ''}>${icon.label}</option>`
                            ).join('')}
                        </select>
                    </div>
                    ...
                    <!-- Gambar -->
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Gambar (URL)</label>
                        ${data && data.image ? `
                            <div class="mb-2">
                                <img src="${data.image}" class="w-32 h-20 object-cover rounded-xl border border-slate-200" alt="Image">
                                <p class="text-xs text-slate-400 mt-1">Gambar saat ini</p>
                            </div>
                        ` : ''}
                        <input type="text" id="formImage" value="${data ? data.image : ''}" 
                            class="w-full rounded-xl border border-slate-200 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-slate-900/10"
                            placeholder="https://...">
                        <p class="text-xs text-slate-400 mt-1">URL gambar untuk ilustrasi peristiwa</p>
                    </div>
                </div>
        ...
        document.getElementById("sejarahForm").onsubmit = saveData;
    }
    ...
    function saveData(e) {
        ...
        if (!year || !title || !description) {
            showToast('⚠️ Tahun, Judul, dan Deskripsi wajib diisi!', 'warning');
            return;
        }

        const data = { year, title, description, icon, image: image || '' };
        ...
        if (currentEditId) {
            ...
            if (index !== -1) {
                ...
                applyFilters();
                showToast(`✅ Timeline "${title}" berhasil diupdate`, 'success');
                closeFormModal();
            }
        } else {
            ...
            applyFilters();
            showToast(`✅ Timeline "${title}" berhasil ditambahkan`, 'success');
            closeFormModal();
        }
    }
    ...
    function confirmDelete() {
        ...
        if (index !== -1) {
            ...
            applyFilters();
            showToast(`🗑️ Timeline "${title}" berhasil dihapus`, 'warning');
            closeDeleteModal();
        }
    }
    ...
    <div class="relative z-10 flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-hidden animate-[fadeIn_.2s_ease]">
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 flex-shrink-0">
                <h2 id="formModalTitle" class="text-xl font-bold text-slate-900">Tambah Timeline Sejarah</h2>
