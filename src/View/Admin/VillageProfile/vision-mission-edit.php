...
<div class="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8">
    <div>
        ...
        <p class="mt-2 text-slate-500 max-w-2xl text-sm lg:text-base leading-relaxed">
            Edit visi atau misi Desa Bungur.
        </p>
    </div>
</div>
...
<div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden max-w-3xl">
    <?php $item = $model['item']; ?>
    <form action="/admin/profile/visions-missions/edit/<?= $item->id ?>" method="POST">
        <div class="p-6 space-y-6">
            ...
            <div>
                ...
                <select name="type"
                    ...
                    <option value="vision" <?= (($model['old']['type'] ?? $item->type ?? '') == 'vision') ? 'selected' : '' ?>>Visi</option>
                    <option value="mission" <?= (($model['old']['type'] ?? $item->type ?? '') == 'mission') ? 'selected' : '' ?>>Misi</option>
                </select>
            ...
            <div>
                ...
                <textarea name="description" rows="6"
                    ...
                    placeholder="Tulis visi atau misi desa..." required><?= htmlspecialchars($model['old']['description'] ?? $item->description ?? '') ?></textarea>
            ...
            <div>
                ...
                <input type="number" name="sort_order" value="<?= htmlspecialchars($model['old']['sort_order'] ?? $item->sortOrder ?? 1) ?>"
                    class="w-full rounded-xl border border-slate-200 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-slate-900/10"
                    placeholder="1" min="1">
