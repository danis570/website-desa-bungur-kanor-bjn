...
<div class="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8">
    <div>
        ...
        <p class="mt-2 text-slate-500 max-w-2xl text-sm lg:text-base leading-relaxed">
            Edit informasi perangkat desa yang sudah ada.
        </p>
    </div>
</div>
...
<div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden max-w-3xl">
    <?php $official = $model['official']; ?>
    <form action="/admin/profile/officials/edit/<?= $official->id ?>" method="POST" enctype="multipart/form-data">
        <div class="p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name"
                    value="<?= htmlspecialchars($model['old']['name'] ?? $official->name ?? '') ?>"
            ...
            <div>
                ...
                <select name="position"
                    ...
                    <option value="Kepala Desa" <?= (($model['old']['position'] ?? $official->position ?? '') == 'Kepala Desa') ? 'selected' : '' ?>>Kepala Desa</option>
                    <option value="Sekretaris Desa" <?= (($model['old']['position'] ?? $official->position ?? '') == 'Sekretaris Desa') ? 'selected' : '' ?>>Sekretaris Desa</option>
                    <option value="Kaur Keuangan" <?= (($model['old']['position'] ?? $official->position ?? '') == 'Kaur Keuangan') ? 'selected' : '' ?>>Kaur Keuangan</option>
                    <option value="Kaur Umum" <?= (($model['old']['position'] ?? $official->position ?? '') == 'Kaur Umum') ? 'selected' : '' ?>>Kaur Umum</option>
                    <option value="Kaur Perencanaan" <?= (($model['old']['position'] ?? $official->position ?? '') == 'Kaur Perencanaan') ? 'selected' : '' ?>>Kaur Perencanaan</option>
                    <option value="Kasi Pemerintahan" <?= (($model['old']['position'] ?? $official->position ?? '') == 'Kasi Pemerintahan') ? 'selected' : '' ?>>Kasi Pemerintahan</option>
                    <option value="Kasi Kesejahteraan" <?= (($model['old']['position'] ?? $official->position ?? '') == 'Kasi Kesejahteraan') ? 'selected' : '' ?>>Kasi Kesejahteraan</option>
                    <option value="Kasi Pelayanan" <?= (($model['old']['position'] ?? $official->position ?? '') == 'Kasi Pelayanan') ? 'selected' : '' ?>>Kasi Pelayanan</option>
                    <option value="Staf Desa" <?= (($model['old']['position'] ?? $official->position ?? '') == 'Staf Desa') ? 'selected' : '' ?>>Staf Desa</option>
                </select>
            ...
            <!-- Periode -->
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Periode <span
                        class="text-red-500">*</span></label>
                <input type="text" name="period"
                    value="<?= htmlspecialchars($model['old']['period'] ?? $official->period ?? '') ?>"
            ...
            <div>
                ...
                <?php if (!empty($official->photo)): ?>
                    <div class="mb-2">
                        <img src="/uploads/official/<?= htmlspecialchars($official->photo) ?>"
                            ...
                            onerror="this.src='https://ui-avatars.com/api/?name=<?= urlencode($official->name) ?>&size=80&background=15803d&color=fff'">
                        <p class="text-xs text-slate-400 mt-1">Gambar saat ini</p>
                    </div>
                <?php else: ?>
                    <div class="mb-2">
                        <img src="https://ui-avatars.com/api/?name=<?= urlencode($official->name) ?>&size=80&background=15803d&color=fff"
                            class="w-20 h-20 rounded-full object-cover border-2 border-slate-200"
                            alt="<?= htmlspecialchars($official->name) ?>">
                        <p class="text-xs text-slate-400 mt-1">Avatar otomatis</p>
                    </div>
                <?php endif; ?>
                ...
                <p class="text-xs text-slate-400 mt-1">Upload foto baru untuk mengganti (JPG, PNG, WEBP, GIF) - Maksimal
                    2MB</p>
            ...
            <div class="bg-slate-50 rounded-xl p-4 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="whatsapp"
                        value="<?= htmlspecialchars($model['old']['whatsapp'] ?? $official->whatsapp ?? '') ?>"
                ...
                <div>
                    ...
                    <input type="url" name="facebook"
                        value="<?= htmlspecialchars($model['old']['facebook'] ?? $official->facebook ?? '') ?>"
                        class="w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10"
                ...
                <div>
                    ...
                    <input type="email" name="email"
                        value="<?= htmlspecialchars($model['old']['email'] ?? $official->email ?? '') ?>"
                        class="w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10"
                ...
                <div>
                    ...
                    <input type="text" name="address"
                        value="<?= htmlspecialchars($model['old']['address'] ?? $official->address ?? '') ?>"
                        class="w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10"
                ...
                <div>
                    ...
                    <input type="text" name="maps_embed_url"
                        value="<?= htmlspecialchars($model['old']['maps_embed_url'] ?? $official->mapsEmbedUrl ?? '') ?>"
                        class="w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10"
            ...
            <div>
                ...
                <select name="is_active"
                    class="w-full rounded-xl border border-slate-200 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-slate-900/10">
                    <option value="1" <?= (($model['old']['is_active'] ?? $official->isActive ?? true) == 1) ? 'selected' : '' ?>>Aktif</option>
                    <option value="0" <?= (($model['old']['is_active'] ?? $official->isActive ?? true) == 0) ? 'selected' : '' ?>>Nonaktif</option>
                </select>
            </div>
