...
<div class="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8">
    <div>
        ...
        <p class="mt-2 text-slate-500 max-w-2xl text-sm lg:text-base leading-relaxed">
            Edit informasi UMKM yang sudah ada.
        </p>
    </div>
</div>
...
<div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
    <?php $umkm = $model['umkm']; ?>
    <form action="/admin/umkm/edit/<?= $umkm->id ?>" method="POST" enctype="multipart/form-data">
        <div class="p-6 space-y-6">
            ...
            <div>
                ...
                <select name="category_id"
                    ...
                    <?php foreach ($model['categories'] as $category): ?>
                        <option value="<?= $category->id ?>" <?= (($model['old']['category_id'] ?? $umkm->categoryId ?? 0) == $category->id) ? 'selected' : '' ?>>
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                    class="w-full rounded-xl border border-slate-200 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-slate-900/10"
                    placeholder="Masukkan deskripsi UMKM..."><?= htmlspecialchars($model['old']['description'] ?? $umkm->description ?? '') ?></textarea>
            </div>
            ...
            <div>
                ...
                <div id="menuContainer" class="space-y-3">
                    <?php if (!empty($umkm->menus)): ?>
                        <?php foreach ($umkm->menus as $index => $menu): ?>
            ...
            <div class="bg-slate-50 rounded-xl p-4 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="business_hours"
                        value="<?= htmlspecialchars($model['old']['business_hours'] ?? $umkm->businessHours ?? '') ?>"
                ...
                <div>
                    ...
                    <input type="text" name="whatsapp"
                        value="<?= htmlspecialchars($model['old']['whatsapp'] ?? $umkm->whatsapp ?? '') ?>"
                        class="w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10"
                ...
                <div>
                    ...
                    <input type="text" name="maps_embed_url"
                        value="<?= htmlspecialchars($model['old']['maps_embed_url'] ?? $umkm->mapsEmbedUrl ?? '') ?>"
                        class="w-full rounded-xl border border-slate-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10"
