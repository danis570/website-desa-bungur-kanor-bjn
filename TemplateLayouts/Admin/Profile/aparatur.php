...
<script>
    ...
    const allData = [
        {
            ...
            photo: "https://ui-avatars.com/api/?name=Ahmad+Fauzi&size=80&background=15803d&color=fff",
            wa: "6281234567890",
            facebook: "https://facebook.com/",
            email: "kades@desa.id",
            address: "Kantor Desa Bungur",
            is_active: true
        },
        {
            id: 2,
            ...
            photo: "https://ui-avatars.com/api/?name=Budi+Santoso&size=80&background=2563eb&color=fff",
            wa: "6281234567891",
            facebook: "https://facebook.com/",
        ...
        },
        {
            ...
            photo: "https://ui-avatars.com/api/?name=Siti+Rahayu&size=80&background=7c3aed&color=fff",
            wa: "6281234567892",
            facebook: "https://facebook.com/",
            email: "kaurkeu@desa.id",
            address: "Kantor Desa Bungur",
            is_active: true
        },
        {
            ...
            photo: "https://ui-avatars.com/api/?name=Ahmad+Fauzi&size=80&background=dc2626&color=fff",
            wa: "6281234567893",
            facebook: "https://facebook.com/",
            email: "kasi@desa.id",
            address: "Kantor Desa Bungur",
        ...
        },
        {
            ...
            name: "Dewi Lestari",
            position: "Staf Desa",
            photo: "https://ui-avatars.com/api/?name=Dewi+Lestari&size=80&background=6b7280&color=fff",
            wa: "6281234567894",
            facebook: "https://facebook.com/",
            email: "staf@desa.id",
            address: "Kantor Desa Bungur",
    ...
    function PhotoRenderer(params) {
        return `<img src="${params.value}" class="w-10 h-10 rounded-full object-cover border-2 border-slate-200" alt="Photo">`;
    }
    ...
    function PositionRenderer(params) {
        ...
        const cls = map[params.value] || 'position-other';
        return `<span class="position-badge ${cls}">${params.value}</span>`;
    }

    function StatusRenderer(params) {
        if (params.value) {
            return `<span class="inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-700">
                <span class="w-1.5 h-1.5 rounded-full bg-green-600"></span>
                Aktif
            </span>`;
        }
        return `<span class="inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-700">
    ...
    const columnDefs = [
        ...
        { headerName: "WhatsApp", field: "wa", width: 140, minWidth: 120, resizable: true },
        { headerName: "Email", field: "email", width: 200, minWidth: 160, resizable: true },
    ...
    function openFormModal(mode, data = null) {
        ...
        if (mode === 'add') {
            currentEditId = null;
            title.textContent = 'Tambah Aparatur Desa';
        } else {
            currentEditId = data.id;
            title.textContent = 'Edit Aparatur Desa';
        }
    ...
    function renderForm(data) {
        ...
        const positions = ['Kepala Desa', 'Sekretaris Desa', 'Kaur Keuangan', 'Kaur Umum', 'Kaur Perencanaan', 'Kasi Pemerintahan', 'Kasi Kesejahteraan', 'Kasi Pelayanan', 'Staf Desa'];

        container.innerHTML = `
            <form id="aparaturForm" class="p-6">
                <div class="space-y-4">
                    <!-- Nama -->
                    <div>
                    ...
                    <div>
                        ...
                        <select id="formPosition" 
                            class="w-full rounded-xl border border-slate-200 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-slate-900/10">
                            ${positions.map(pos =>
            `<option value="${pos}" ${data && data.position === pos ? 'selected' : ''}>${pos}</option>`
        ).join('')}
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        ${data && data.photo ? `
                            <div class="mb-2">
                                <img src="${data.photo}" class="w-16 h-16 rounded-full object-cover border-2 border-slate-200" alt="Photo">
                                <p class="text-xs text-slate-400 mt-1">Gambar saat ini</p>
                            </div>
                        ` : ''}
                        <input type="text" id="formPhoto" value="${data ? data.photo : ''}" 
                            class="w-full rounded-xl border border-slate-200 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-slate-900/10"
                            placeholder="https://ui-avatars.com/api/?name=Nama&size=80&background=15803d&color=fff">
                        <p class="text-xs text-slate-400 mt-1">Gunakan UI Avatars atau URL gambar</p>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="url" id="formFacebook" value="${data ? data.facebook : ''}" 
                            class="w-full rounded-xl border border-slate-200 px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-slate-900/10"
        ...
        document.getElementById("aparaturForm").onsubmit = saveData;
    }
    ...
    function saveData(e) {
        ...
        const photo = document.getElementById("formPhoto").value.trim() || `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&size=80&background=15803d&color=fff`;
        const wa = document.getElementById("formWa").value.trim();
        const facebook = document.getElementById("formFacebook").value.trim();
        ...
        const data = { name, position, photo, wa, facebook, email, address, is_active };
        ...
        if (currentEditId) {
            ...
            if (index !== -1) {
                ...
                applyFilters();
                showToast(`✅ Data "${name}" berhasil diupdate`, 'success');
                closeFormModal();
            }
        } else {
    ...
    function confirmDelete() {
        ...
        if (index !== -1) {
            ...
            applyFilters();
            showToast(`🗑️ Data "${name}" berhasil dihapus`, 'warning');
            closeDeleteModal();
        }
    }
    ...
    <div class="relative z-10 flex items-center justify-center min-h-screen p-4">
        <div
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 flex-shrink-0">
                <h2 id="formModalTitle" class="text-xl font-bold text-slate-900">Tambah Aparatur Desa</h2>
